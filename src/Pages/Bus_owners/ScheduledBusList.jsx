...
const ScheduledBusList = () => {
  ...
  useEffect(() => {
    ...
    axiosInstance.get('/busowner-dashboard/scheduled-buses/', {
      headers: {
  ...
  return (
    <div className="container mx-auto p-6">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-3xl font-semibold text-gray-800">Scheduled Bus List</h1>
      </div>
        ...
        <table className="min-w-full table-auto border-collapse border border-red-300">
          ...
          <tbody>
            {scheduledBuses.length === 0 ? (
              <tr>
                <td colSpan="5" className="px-4 py-2 text-center text-sm text-gray-500">No scheduled buses available.</td>
              </tr>
            ) : (
              scheduledBuses.map(bus => (
                <tr key={bus.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-2 text-sm">
                    <button
                      onClick={()=>navigate(`/busowner-dashboard/scheduled-bus-details/${bus.id}`)}
